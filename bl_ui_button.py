...
class BL_UI_Button(BL_UI_Patch): 
    ...
    def draw_text(self):
        ...
        length1 = blf.dimensions(0, self._text)
        height1 = blf.dimensions(0, "W")  # This is to keep a regular pattern since letters differ in height
        ...
        if self._textwo != "":
            ...
            scaled_size = self.over_scale(leveraged_text_size)
            blf.size(0, scaled_size, 72)
            length2 = blf.dimensions(0, self._textwo)
            height2 = blf.dimensions(0, "W")  # This is to keep a regular pattern since letters differ in height
        else:
            length2 = [0,0]
            height2 = [0,0]
        ...
        top_margin = int((self.over_scale(self.height) - round(height1[1]+0.499) - round(height2[1]+0.499) - self.over_scale(middle_gap)) / 2.0)
        
        textpos_y = self.over_scale(self._textpos[1]) + top_margin + round(height1[1]+0.499) - 1
        ...
        if self._text != "":
            textpos_x = self.over_scale(self._textpos[0]) + int((self.over_scale(self.width) - round(length1[0])) / 2.0) - 1

            label = BL_UI_Label(int(textpos_x/over_scale), int(textpos_y/over_scale), length1[0], height1[1])
            label.style = 'BUTTON'
            
            label.text = self._text
            label.text_size = text_size        # <<-  Always send the original size as it will be leveraged and scaled in there
            ...
            textpos_y = textpos_y + self.over_scale(middle_gap) + round(height2[1]+0.499)

        if self._textwo != "":
            textpos_x = self.over_scale(self._textpos[0]) + int((self.over_scale(self.width) - length2[0]) / 2.0) - 1

            label = BL_UI_Label(int(textpos_x/over_scale), int(textpos_y/over_scale), length2[0], height2[1])
            label.style = 'BUTTON'
            
            label.text = self._textwo
            label.text_size = textwo_size      # <<-  Always send the original size as it will be leveraged and scaled in there
            label.text_kerning = text_kerning
