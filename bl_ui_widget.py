...
class BL_UI_Widget():
    ...
    def __init__(self, x, y, width, height):
        ...
        self._is_tooltip = False    # Indicates whether the object generated by this class is of 'Tooltip' type
        ...
        self.__tooltip_shifted = False # Indicates whether the container panel has been dragged to other position
    ...
    def RC_PINNED(self):
        ...
        """
        return (bpy.context.preferences.addons[__package__].preferences.RC_PINNED)

    def RC_SCALE(self):
        """ Scaling to be applied on the Remote Control panel  
            over (in addition to) the interface ui_scale.
        """
        return (bpy.context.preferences.addons[__package__].preferences.RC_SCALE)
    ...
    def leverage_text_size(self, text_size, style):
        ...
        if style == "panel_title":
            factor = (style_size - 12) / 12  # Should be 11 for 'Deep Gray', 'Minimal Dark' and 'Modo' v2.93 themes
        elif style == "widget":
            factor = (style_size - 11) / 11  # Should be 10 for 'Minimal Dark' and 'Modo' v2.93 themes
        elif style == "widget_label":
            factor = (style_size - 11) / 11  # Should be 10 for 'Minimal Dark' v2.93 theme
    ...
    def update(self, x, y):
        ...
        if self._radius > 10:  #self._radius == 0 or self._radius > 10 or self._rounded_corners == (0,0,0,0):
    ...
    def verify_screen_position(self, area_height):        
        if self._is_tooltip:
            ...
            base_class = super().__thisclass__.__mro__[-2]  
            widget = base_class.g_tooltip_widget
            if not widget.__tooltip_current:
                self.update(0,0)
    ...
    def halt_tooltip(self):
        ...
        if time.time() - widget.__tooltip_gotimer < 1:
            # Timer has not reached the threshold
            return True
    ...
    def draw(self):
        ''' Note: 
            This function is used by BL_UI_Drag_Panel, BL_UI_Patch, BL_UI_Button by BL_UI_Tooltip classes.
    ...
    def set_colors(self):
        if not (self._bg_color is None):
            ...
            # Invisible (good for icons and images)
            bgColor = (0,0,0,0) 
    ...
    def draw_text(self):
        ...
        # so the full function is in their py module
        pass
    ...
    def calc_corners_for_trifan(self, x_screen, y_screen, width, height, radius, selection):
        ...
        if use_rounded_corners != (0,0,0,0):
            ...
            # Alternative Method (not much good)
            # x = 0
            # y = r
            # d = int(3 - 2*r)
    ...
    def calc_corners_for_lines(self, x_screen, y_screen, width, height, radius, selection):
        ''' 
            Clockwise coordinates: (bottom left, top left, top right, bottom right).
            Argument selection: value can be 'FULL', 'OUTLINE-A', 'OUTLINE-B', 'SHADOW-A' or 'SHADOW-B': 
    ...
    def _get_mapped_coords(self, radius):
        ''' 
            ...
            manually created by me and they should always give a nice contour. May GOD forgive me!
        '''
        map = [( 0,),
               ( 1,0),
               ( 2,0,1),
